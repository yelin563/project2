...
with st.form("my_form"):
    header = st.columns([1])
    header[0].subheader('1) 우리 동네 대피소는 어디일까? :mag:')
    ...
    st.form_submit_button('대피소 찾기', on_click=click_button, args=("button21",))
...
if st.session_state.button21:
    ...
    folium_static(m1)
    st.subheader("대피소 정보 확인하기")
    
    st.write("대피소 이름을 입력하여 대피소 정보를 확인해봅시다.")
    sheltername=st.text_input("대피소 클릭 후 보이는 대피소 이름을 복사, 붙여넣기 해보세요")
...
if st.session_state.button26:
    ...
    if len(sh_d)==0:
        st.write('대피소 이름을 다시 확인해주세요!:sweat_smile:')
    else:
        st.write(sh_d)
        st.write('가까운 대피소의 정보를 모두 확인해보세요.')
        st.write('수용인원, 대피소 유형, 거리 등을 고려하여 어디로 대피를 해야할지 결정해봅시다')
...
if st.session_state.button22:
    ...
    st.subheader('2) 거리를 측정하고 걸리는 시간 구해보자:hourglass_flowing_sand:')
        
    st.write('대피 경로를 고려하여 거리를 구해봅시다')
    ...
    with st.form("form2"):
        mydist=st.text_input("대피소까지의 거리(m)를 입력해주세요.",placeholder="단위(m)를 제외하고 숫자만 입력해주세요.")
        mytime=st.text_input("빨리 걸을 때 여러분의 속력이 5km/h이라면 대피할 때 걸리는 시간은 몇 분인가요?",placeholder="걸리는 시간은 자연수로 입력해주세요. 약간의 오차는 인정됩니다.")
...
if st.session_state.button23:
    ...
    if int(mydist/500*6)<=mytime<=int(mydist/500*6)+1:
        st.write("거리와 속력으로 대피 시간을 잘 계산했네요! :ok_hand: 다음 활동을 진행해주세요.")
        st.button('다음 활동 진행하기', on_click=click_button, args=("button24",))
    else:
        st.write("대피 시간을 다시 계산해봅시다! :sweat_smile: :blue[단위]를 주의해주세요")
...
if st.session_state.button24:
    st.divider()
    st.subheader("3) 대피소까지의 매뉴얼 작성해보자:writing_hand:")
    ...
    st.subheader('4) 지역별로 대피소 유형은 어떨까:question:')
