...
st.title("3.소방서:fire_engine:")

st.divider()
st.subheader('1) 가장 가까운 119안전센터는 어디일까:question:')
...
with st.form("form"):
            mydist=st.text_input("구급대가 해당 119안전센터에서 출발했을때 집까지의 구급대의 예상 이동거리를 입력해주세요",placeholder="단위(m)를 제외하고 숫자만 입력해주세요.")
...
if st.session_state.button31:
    ...
    if int(mydist/2000*6)<=mytime<=int(mydist/2000*6)+1:
        st.write("거리와 속력으로 구급대의 출동 시간을 잘 계산했네요! :thumbsup: 다음 활동을 진행해주세요.")
        
        st.button('다음 활동 진행하기', on_click=click_button, args=("button32",))
    else:
        st.write("시간을 다시 계산해봅시다! :confused: 단위를 주의해주세요")
...
if st.session_state.button32:
    ...
    st.subheader('2) 구별 119안전센터 수의 출동건수를 알아보자 :rotating_light:')
    st.write("구별로 119안전센터가 얼마나 많을까:question:")
    ...
    st.write('구별로 구급,구조 출동건수가 얼마나 많을까:question:')
...
if st.session_state.button33:
    if an1=='강남구' and an2==6:
        if an3=='금천구' and an4==2:
            st.write("지도를 잘 해석했네요!	:clap: 다음 활동으로 진행해주세요")
            st.button("다음 활동 진행하기", on_click=click_button, args=("button34",),key='b1')
        else:
            st.write("Q2를 다시 확인해주세요!:fearful:")
    else:
        if an3=='금천구' and an4==2:
            st.write("Q1를 다시 확인해주세요!:open_mouth:")
        else:
            st.write("지도를 다시 해석해봅시다.	:stuck_out_tongue_closed_eyes: 지도 왼쪽 위에 있는 선의 색깔을 잘 봐주세요.")
...
if st.session_state.button34:  
    st.divider()
    st.subheader('3) 119안전센터의 갯수와 출동건수를 확인해보자:rotating_light:')
    ...
    with st.form("form2"):
        ...
        st.write("**119안전센터의** **갯수**를 x좌표로 표현하고, **출동건수**를 y좌표로  표현해봅시다")
...
if st.session_state.button35:
    ...
    st.subheader(":bulb:구 별로 안전센터의 개수와 출동 건수가 다른 이유는 무엇일까요?") 
...
if st.session_state.button36:  
    ...
    st.write('인구가 많은 지역에 119안전센터를 많이 짓고 인구가 많은 지역이 출동건수가 많기 때문이었네요!')
    
    st.write('구의 인구에 맞춰 119안전센터를 지었으니 119안전센터별 출동건수는 모두 비슷할까요?')
    st.write('119안전센터가 가장 바쁜 구와 가장 여유로운 구는 어디일까요:question:')
    st.divider()
    st.subheader("4) 119안전센터 당 출동건수를 알아보자:male-firefighter:")
    with st.form("form3"):
        st.write("각 구 별로 어떤 계산을 해봐야 할까요:question:")
...
if st.session_state.button37:
    if a1=='출동건수' and a2=='나누기' and a3=='119 안전센터 갯수':
        st.write('맞아요!:clap: 그러면 구 별 119 안전센터 당 출동건수를 살펴볼까요?')
        ...
        fg = folium.FeatureGroup(name="119안전센터위치", show=False).add_to(m3)
        df119loc.apply(lambda row:Marker(location=[row["위도"],row["경도"]],icon=folium.Icon(color='green'),
                                popup=row['센터명']).add_to(fg),axis=1)
        ...
        ans2=st.text_input("서울에 새로운 119안전센터를 지으면 좋을 곳을 적고 이유를 적어주세요")
        st.write(':blue["숭실대근처"]처럼 구체적으로 적어주세요')
