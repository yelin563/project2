...
st.title('안전매뉴얼 모둠활동:people_holding_hands:')
school_lat=37.472
school_lon=127.092
...
if x=='응급실과 119안전센터':
    ...
    st.subheader("1) 학교에서 가장 가까운 119안전센터는 어디일까?")
    s1=st.text_area("학교에서 친구 혹은 자신이 다쳤을 때의 상황을 구체적으로 설정해봅시다.(시간,이유,장소,주변상황 등)")
    st.write('119에 신고를 했을 때 구급대가 **어떤** **경로**로 **몇** **분** 후에 올지 예상해봅시다')
    ...
    folium_static(m)
    st.subheader("2) 구급차의 출동 시간 확인해보자")
    with st.form("form"):
            mydist=st.text_input("구급대가 해당 119안전센터에서 출발했을때 학교까지의 예상 이동거리를 입력해주세요",placeholder="단위(m)를 제외하고 숫자만 입력해주세요.")
            mytime=st.text_input("구급대의 평균 속력이 20km/h이라면 출동에 걸리는 시간은 몇 분인가요?",placeholder="걸리는 시간은 자연수로 입력해주세요. 약간의 오차는 인정됩니다.")
    ...
    if st.session_state.button41:
        ...
        if int(mydist/2000*6)<=mytime<=int(mydist/2000*6)+1:
            st.write("거리와 속력으로 구급대의 출동 시간을 잘 계산했네요!:sunglasses: 다음 활동을 진행해주세요.")
            
            st.button('다음 활동 진행하기', on_click=click_button, args=("button42",))
        else:
            st.write("시간을 다시 계산해봅시다!	:neutral_face: 단위를 주의해주세요")
    if st.session_state.button42:
        st.subheader("3) 응급 처치를 해보자")
        manual1=st.text_area("구급대가 도착할 때까지의 어떤 응급 처치를 해야 할 지 생각해봅시다. 제출 후 다음 활동 진행하기 버튼을 눌러주세요.")
    ...
    if st.session_state.button43:
        st.subheader("4) 학교 근처 응급실은 어디인가?")
    ...
    if st.session_state.button44:
        ...
        with st.form("form2"):
            ...
            mydist1=st.text_input("응급실까지의 거리(m)를 입력해주세요.",placeholder="단위(m)를 제외하고 숫자만 입력해주세요.")
            mytime1=st.text_input("구급대의 평균 속력이 20km/h이라면 응급실까지 걸리는 시간은 몇 분인가요?",placeholder="걸리는 시간은 자연수로 입력해주세요. 약간의 오차는 인정됩니다.")
    ...
    if st.session_state.button45:
        ...
        if int(mydist1/2000*6)<=mytime1<=int(mydist1/2000*6)+1:
            st.write("거리와 속력으로 응급실까지 가는 시간을 잘 계산했네요!	:smile: 다음 활동을 진행해주세요.")
            
            st.button('다음 활동 진행하기', on_click=click_button, args=("button46",),key='b46')
        else:
            st.write("시간을 다시 계산해봅시다!	:neutral_face: 단위를 주의해주세요")
    if st.session_state.button46:
        st.subheader("5) 우리학교 안전수칙:warning:")
        st.write("우리의 안전은 우리가 지킨다!:superhero: 우리 학교 근처 응급실, 119안전센터를 살펴보며 위급 상황을 대비해보았습니다.")
        st.write("위급 상황일 때 당황하지 않을 수 있겠죠? 하지만 다치지 않는 것이 가장 최선이라는 것 잊지 말아요!")
        an3=st.text_area("우리 학교에서 지켜야 할 안전 수칙을 3가지 적어봅시다")
        button6=st.button('제출하기', on_click=addresponse , args=('조)학교 안전수칙',an3),key='b6')
        if button6:
            st.write(":teacher:모둠 활동이 끝났습니다. 첫 페이지로 돌아가 파일을 다운 받고 제출해주세요")
...
if x=='대피소':
    ...
    st.subheader("1) 학교에서 가장 가까운 대피소는 어디일까?")
    ...
    if st.session_state.button47:
        ...
        with st.form("form2"):
            mydist2=st.text_input("대피소까지의 거리(m)를 입력해주세요.",placeholder="단위(m)를 제외하고 숫자만 입력해주세요.")
            mytime2=st.text_input("빨리 걸을 때 여러분의 속력이 5km/h이라면 대피할 때 걸리는 시간은 몇 분인가요?",placeholder="걸리는 시간은 자연수로 입력해주세요. 약간의 오차는 인정됩니다.")
    ...
    if st.session_state.button48:
        ...
        if int(mydist2/500*6)<=mytime2<=int(mydist2/500*6)+1:
            st.write("거리와 속력으로 대피 시간을 잘 계산했네요!:blush 다음 활동을 진행해주세요.")
            st.button('다음 활동 진행하기', on_click=click_button, args=("button49",))
        else:
            st.write("대피 시간을 다시 계산해봅시다!:disappointed: 단위를 주의해주세요")
    if st.session_state.button49:
        st.subheader("4) 대피 매뉴얼 작성 하기:writing_hand:")  
        ...
        if button6:
            st.write("우리의 안전은 우리가 지킨다!:superhero: 우리 학교 근처 대피소를 살펴보며 위급 상황을 대비해보았습니다.")
            st.write("위급 상황일 때 당황하지 않을 수 있겠죠?")
            st.write(":teacher: 모둠 활동이 끝났으니 첫 페이지로 돌아가 파일을 다운 받고 제출해주세요")